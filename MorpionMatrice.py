...
def gagne():
    ...
    for ligne in range(1, 4):
        if somme_colonne(L, ligne)==3 or somme_colonne(L, ligne)==-3:
            return True
    for colonne in range(1, 4):
        if somme_colonne(L, colonne)==3 or somme_colonne(L, colonne)==-3:
            return True
    for diag1 in range(1, 4):
        if somme_diag_1(L, diag1)==3 or somme_diag_1(L, diag1)==-3:
            return True
    for diag2 in range(1, 4):
        if somme_diag_2(L, diag2)==3 or somme_diag_2(L, diag2)==-3:
            return True
    return False
...
def matchnul():
    ...
    if Gagne==False and ("⚫" not in L): # Si personne n'a gagné et grille pleine
